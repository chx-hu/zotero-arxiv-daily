...
- Support DeepSeek and Volcengine for generating bilingual TLDR.
...
| MAX_JOURNAL_NUM |          | int  | Max number of journal papers.                                                                                                                                                                                                                                                                                          | 10                            |
...
| MAX_PAPER_NUM   |          | int  | Maximum number of arXiv papers presented in the email. This value directly affects execution time, because TL;DR generation and source parsing are expensive. `-1` means present all retrieved arXiv papers.                                                                                                      | 10                            |
| MAX_BIORXIV_NUM |          | int  | Max number of bioRxiv papers.                                                                                                                                                                                                                                                                                          | 10                            |
...
| JOURNAL_LOOKBACK_DAYS |          | int  | Number of previous UTC calendar days to include in addition to the scheduled UTC trigger day. `1` means “scheduled day + previous day”.                                                                                                                                                           | `1`                                         |
| SCHEDULE_UTC_HOUR | | int | Scheduled trigger hour in UTC used for date-window calculations. Keep this aligned with your workflow cron. Defaults to `20` for a Beijing-time `04:00` schedule. | `20` |
| SCHEDULE_UTC_MINUTE | | int | Scheduled trigger minute in UTC used for date-window calculations. Keep this aligned with your workflow cron. Defaults to `0`. | `0` |
...
> [!IMPORTANT]
> TLDR generation uses DeepSeek when `DEEPSEEK_API_KEY` is set, otherwise Volcengine when `VOLCENGINE_API_KEY` is set. Without either key, TLDR generation is skipped.
...
*Zotero-arXiv-Daily* firstly retrieves all the papers in your Zotero library and all the papers released in the configured UTC date window, via corresponding API. It now supports three source buckets: arXiv, bioRxiv, and configured journals. arXiv, bioRxiv, and journals all use the same scheduled-UTC reference window: the planned workflow trigger day plus the configured number of previous UTC days, rather than the possibly delayed actual runner start time. Journal retrieval uses publisher-direct pages/feeds where possible, and official ScienceDirect article retrieval for supported Elsevier journals. Then it calculates the embedding of each paper's abstract via an embedding model. The score of a paper is its weighted average similarity over all your Zotero papers (newer paper added to the library has higher weight).

The TLDR of each paper is generated by the configured LLM provider, given its title, abstract, introduction, and conclusion (if any). The introduction and conclusion are extracted from the source latex file of the paper. The workflow asks the model to return a one-sentence English TLDR and a Chinese summary of approximately 150-200 Chinese characters in one JSON response. The Chinese summary covers the research question, core method, and main findings while preserving qualifications; this is a prompt-level length target, not a hard truncation. The email currently renders only the Chinese summary.

## 📌 Limitations

- The recommendation algorithm is very simple, it may not accurately reflect your interest. Welcome better ideas for improving the algorithm!
- TLDR generation depends on the configured LLM API. High per-source limits such as `MAX_PAPER_NUM`, `MAX_BIORXIV_NUM`, and `MAX_JOURNAL_NUM` can still increase runtime, especially because `arXiv` source parsing can be expensive. If you have special requirements, you can deploy the workflow in your own server, or use a self-hosted Github Action runner, or reduce the per-source limits.
...
Both `main.py` and `biorxiv_demo.py` load the local, Git-ignored `.env` file.
Set `DEEPSEEK_API_KEY` there to select DeepSeek automatically, even if a Volcengine key is also configured.
...
DeepSeek requests explicitly disable thinking mode for short TLDRs and affiliation extraction.
`deepseek-flash` is the default; set `DEEPSEEK_MODEL=deepseek-v4-pro` to select Pro instead.
The model is selected from configuration, not automatically benchmarked or switched on failures.
The base URL above is the full chat-completions endpoint because this version uses direct HTTP requests.
See the [official models](https://api-docs.deepseek.com/quick_start/pricing) and
[thinking-mode documentation](https://api-docs.deepseek.com/guides/thinking_mode).

For GitHub Actions, add repository secret `DEEPSEEK_API_KEY`. Both daily and test workflows pass it to the script. Optionally set repository variable `DEEPSEEK_MODEL`; if omitted, `deepseek-flash` is used. The local `.env` must not be committed.
